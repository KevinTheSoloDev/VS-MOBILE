...
#define GLFW_ANDROID_EVENT_TYPE_SCROLL 5

// These are a bitmask; (1 >> 1) evaluated to 0, so FLAG_APP_FOCUS could never
// be set in update_flags.
...
static void android_pump_event() {
    ...
    switch (event.type) {
        ...
        case GLFW_ANDROID_EVENT_TYPE_UNICODE_CHARS:
            ...
            break;
        case GLFW_ANDROID_EVENT_TYPE_SCROLL:
            _glfwInputScroll(surfaceOwner, event.s.x, event.s.y);
            break;
    }
}
...
Java_git_artdeell_dnbootstrap_glfw_GLFW_sendBulkUnicodeEvent(JNIEnv *env, jclass clazz,
    ...
    jchar* codepoints = malloc(length * sizeof(jchar));
...
// Added by the VS-Mobile fork. ThermalManager (Java) needs to change the swap
// interval at runtime to trade frame rate for temperature head-room. This is a
// thin JNI wrapper over glfwSwapInterval; the Java side declares
//   private static native void setSwapInterval(int interval);
// Without this export that call raises UnsatisfiedLinkError. It is present in
// neither branch of VSMobile/glfw, only in the previously committed
// app/.cxx/.../android_window.c.o -- a local patch that was never pushed.
JNIEXPORT void JNICALL
Java_git_artdeell_dnbootstrap_ThermalManager_setSwapInterval(JNIEnv *env, jclass clazz,
                                                             jint interval) {
    glfwSwapInterval((int) interval);
}

// Added by the VS-Mobile fork. GLFW.java declares
//   public static native void sendScrollEvent(double xOffset, double yOffset);
// and ControlButton calls it for the SP_MOUSE_SCROLL_UP/DOWN buttons. It was
// never implemented here, so pressing a scroll button raised
// UnsatisfiedLinkError -- and unlike ThermalManager.setSwapInterval that call
// site has no catch.
JNIEXPORT void JNICALL
Java_git_artdeell_dnbootstrap_glfw_GLFW_sendScrollEvent(JNIEnv *env, jclass clazz,
                                                        jdouble xOffset, jdouble yOffset) {
    input_event_t event = {
            .type = GLFW_ANDROID_EVENT_TYPE_SCROLL,
            .s.x = xOffset,
            .s.y = yOffset
    };
    android_send_event(&event);
